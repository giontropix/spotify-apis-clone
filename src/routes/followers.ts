...
router.put("/followed", body('userIdToFollow').isString().notEmpty(), handleErrors, ({body: {userIdToFollow}, params: {id}}, res: Response) => {
    ...
    currentUser.followed.push(new Follower(userToFollow.id, userToFollow.user_name, userToFollow.sex))
    userToFollow.followers.push(new Follower(currentUser.id, currentUser.user_name, currentUser.sex))
    writeToFile()
    res.status(201).json({message: "User followed", currentUser, userToFollow})
})

//VEDERE CHI SEGUE UN UTENTE
router.get("/followed", ({params: {id}, query: {filter, offset, limit}}: Request, res: Response) => {
    const currentUser = listOfUsers.find((user: User) => user.id === id)
    if (!currentUser) return res.status(404).json({error: "User not found"})
    if(filter){
        let followedToSearch: Follower | Follower[] | undefined = currentUser.followed.filter((foll: Follower) => {
            if (foll.user_name.toLowerCase().includes(String(filter).toLowerCase())) return foll
        });
        if (offset && limit) return res.status(200).json(followedToSearch.slice(Number(offset), Number(offset) + Number(limit)))
        return res.status(200).json(followedToSearch)
    }
